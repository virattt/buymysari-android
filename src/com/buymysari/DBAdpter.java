...
public class DBAdpter {
	...
	public static ArrayList<search_items_dto> getSearchItemsData(
			String searchText, String searchCateId) {
		...
		try {
			...
			// cat_id
			Log.v("log",
					" Search Items length -->  " + SearchItemsArray.length());
		...
		try {
			...
			for (int i = 0; i < j_Array.length(); i++) {
				...
				//	list_home_data.picture = json_objs.getString("picture");
					...
					list_home_data.category_name = json_objs_items
	...
	public static String updateUserInfo(String id, String fname, String lname,
	...
	public static String updateUserStore(String str_user_id, String str_name,
		...
		Log.v("log", " getUserID  "
				+ fetch_UserDetail_data.get(0).getUser_id().toString());
